...
double doFit(RooDataSet& dataset, RooRealVar& response, RooRealVar& pt_jet, TString cut, 
  ...
  RooDataSet* reduced_dataset = (RooDataSet*) dataset.reduce(RooArgSet(response,pt_jet), cut.Data());
...
int main(int argc, char** argv) {
  ...
  RooDataSet _dataset("_dataset", "", _hs_jet);
  ...
  for (int ifile = 1; ifile<=437; ifile++) {
	...
	lep1.SetPtEtaPhiE(el_pt[1]/GeV, el_eta[1], el_phi[1], el_e[1]/GeV);
      ...
      if (jet.Pt() < 20) {
	zee_mass_low->Fill(z.M());
	zee_pt_low->Fill(z.Pt());
	response_low->Fill(response-1);
      } else {
	zee_mass_high->Fill(z.M());
	zee_pt_high->Fill(z.Pt());
	response_high->Fill(response-1);
      }
  ...
  for (int i=0; i<ntot; i++) {
    ...
    _mean.setRange(0.6*meanEstimate-1., 1.2*meanEstimate-1.);
    resolution = doFit(_dataset, _response, _pt_jet, TString::Format("_pt_jet > %.3f && _pt_jet < %.3f", 15.+val*((double) i), 15.+val*(((double) i)+1.)), 
  ...
  RooDataSet* _reduced_dataset_1 = (RooDataSet*) _dataset.reduce(RooArgSet(_response,_pt_jet), "_pt_jet > 15.0 && _pt_jet < 17.5");
  RooDataSet* _reduced_dataset_2 = (RooDataSet*) _dataset.reduce(RooArgSet(_response,_pt_jet), "_pt_jet > 17.5 && _pt_jet < 20.0");
  RooDataSet* _reduced_dataset_3 = (RooDataSet*) _dataset.reduce(RooArgSet(_response,_pt_jet), "_pt_jet > 20.0 && _pt_jet < 22.5");
  RooDataSet* _reduced_dataset_4 = (RooDataSet*) _dataset.reduce(RooArgSet(_response,_pt_jet), "_pt_jet > 22.5 && _pt_jet < 25.0");
  ...
  TFile* outputFile = TFile::Open("plots_data18.root", "RECREATE");
